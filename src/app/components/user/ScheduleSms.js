...
export default function ScheduleSms({ userId, setStatus }) {
  ...
  const fetchQueue = async () => {
    try {
      const res = await fetch(`http://localhost/sms-backend/schedule.php?action=get_scheduled&user_id=${userId}`);
      if (!res.ok) {
  ...
  const handleSchedule = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("http://localhost/sms-backend/schedule.php", {
        method: "POST",
